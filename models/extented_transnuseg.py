...
class WindowAttention_up(nn.Module):
    ...
    def forward(self, x, encoder_attention, mask=None):
        """
        Args:
        ...
        concat_attn = attn + encoder_attention

        x = (concat_attn @ v).transpose(1, 2).reshape(B_, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        attn_windows , attention = self.attn(x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
...
class SwinTransformerBlock_up(nn.Module):
    ...
    def forward(self, x, encoder_attention):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, encoder_attention, mask=self.attn_mask)  # nW*B, window_size*window_size, C
...
class TransNuSeg(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=4, in_chans=3, num_classes=2,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.BR = BoundaryRefinement(in_channels=num_classes)
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers[:-1]:
            ...
            seg_mask, attention = layer(seg_mask)
            edge_mask, attention = layer(edge_mask)
            encoder_attentions.append(attention)
            
        edge_mask_downsample.append(edge_mask)
        seg_mask_downsample.append(seg_mask)
        
        seg_mask = self.layers[-1](seg_mask)
        edge_mask = self.layers[-1](edge_mask)
        ...
        # print('encoder attention ', len(encoder_attentions))
        # print('encoder attention 0',encoder_attentions[0].shape)
        # print('encoder attention 1',encoder_attentions[1].shape)
        # print('encoder attention 2',encoder_attentions[2].shape)

        # print('edge masks 0',edge_mask_downsample[0].shape)
        # print('edge masks 1',edge_mask_downsample[1].shape)
        # print('edge masks 2',edge_mask_downsample[2].shape)
        # print('edge masks 3',edge_mask_downsample[3].shape)
  
        return seg_mask,edge_mask, seg_mask_downsample,edge_mask_downsample, encoder_attentions

    #Dencoder and Skip connection
    def forward_up_features(self, seg_mask,edge_mask, seg_mask_downsample,edge_mask_downsample, encoder_attentions):
        ...
        for inx in range(len(self.layers_up)):
            if inx == 0:
                ...
                seg_mask = self.layers_up[inx](seg_mask , encoder_attentions[3-inx])
    ...
    def forward(self, x):
        seg_mask,edge_mask, seg_mask_downsample,edge_mask_downsample, encoder_attentions = self.forward_features(x)

        # #print("downsampling, ")

        seg_mask,edge_mask,cluster_edge = self.forward_up_features(seg_mask,edge_mask, seg_mask_downsample,edge_mask_downsample, encoder_attentions)

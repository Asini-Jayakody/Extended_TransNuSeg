...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, softmax=False, input_one_hot = False):
        if softmax:
...
def draw_accuracy(train_loss,val_loss,now):
    plt.subplot(2, 1, 2)
    ...
    plt.xlabel('epoch')
    plt.savefig('./log/TrainAccuracy_{}.png'.format(now), dpi=100)
    plt.show()
